...
function saveClientSnapshot(payload) {
  ...
  const snapshot = {
    meta: {
      ...
      forecastDefinition: '月末予測は、45h（法定時間外労働）、60h（月60時間超割増対象・法定休日労働を除く）、80h（時間外＋休日労働）を別々の実績値で、現時点実績 + (現時点実績 ÷ 経過日数（出社日数＋対象月の年次有休取得日数）) × 残営業日数により算出します。見込みは小数第1位切り上げ、超過判定は > で行います。',
